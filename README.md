...
项目当前已完成邮件入站、工单流转、知识库自动解决、人工接管和 Agent Trace。后续会继续扩展更细粒度的业务动作审批，例如补发、改订单、赔偿、账号安全核验等场景，将 Agent 生成的处理建议和人工确认流程沉淀为更完整的审批队列。

## Agentic Workflow 设计

我没有把 AI 做成一个单次问答入口，而是把它嵌入到客服工单生命周期中，形成可追踪、可接管、可审计的 Agentic workflow：

- Triage Agent 负责工单分类，减少人工分流成本。
- Knowledge Agent 通过 Jina embeddings + pgvector 检索知识库，为回复和决策提供依据。
- Supervisor Agent 判断知识库是否足以支撑自动处理，避免模型在缺少依据时直接处理个案。
- Action Agent 只在低风险、知识库充分的情况下发送邮件并关闭工单；退款类命中策略会被 Supervisor 拦截并转人工。
- Handoff Agent 将不确定、高风险或失败工单转回人工队列，保留客服接管入口。
- AgentRun / AgentStep 记录每次自动化处理的关键步骤，用于审计、排障和效果评估。
