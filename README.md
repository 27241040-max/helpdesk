...
当前实现聚焦工单自动分流和知识库可自动解决场景；退款类工单已有强制人工审批边界，补发、改订单、赔偿等更多业务动作尚未接入独立工具和审批队列。邮件入站采用 SendGrid inbound webhook，不是 IMAP 监听。

## 面试讲解定位

这个项目不是单次问答机器人，而是一个可治理的 Agentic workflow：

- Triage Agent 负责分类，减少人工分流成本。
- Knowledge Agent 通过 Jina embeddings + pgvector 检索知识库，保证回复有依据。
- Supervisor Agent 判断知识库是否足以自动解决，避免模型在缺少依据时直接处理个案。
- Action Agent 只在低风险、知识库充分的情况下发送邮件并关闭工单；退款类命中策略会被 Supervisor 拦截并转人工。
- Handoff Agent 将不确定、高风险或失败工单转回人工队列。
- AgentRun / AgentStep 让每次自动化处理可审计、可排障、可向业务方解释。
